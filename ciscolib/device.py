...
class Device(object):
    ...
    def cmd_ex(self, cmd_text):
        ...
        text = ""
        while 1:
            index = self.expect(["---More---", "#", ">"], 5)
            # print(index)
            text = text + index[2].replace("--More--", "")\
                .replace("\x08", "")\
                .replace(cmd_text, "")\
                .replace(self.hostname + "#", "")\
                .replace(self.hostname + ">", "")
        ...
        if "Invalid input" in text or "Incomplete command" in text:
            raise InvalidCommand(cmd_text)
